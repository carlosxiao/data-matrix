function do_js_beautify(el, code) {
  js_source = code.replace(/^\s+/, '') || document.getElementById(el).value.replace(/^\s+/, '');
  tabsize = 4;
  tabchar = ' ';
  if (js_source && js_source.charAt(0) === '<') {
    document.getElementById(el).value = style_html(js_source, tabsize, tabchar, 80)
  } else {
    document.getElementById(el).value = js_beautify(js_source, tabsize, tabchar)
  }
  return false
}
